...
│   ├── agent.py               # Coordinator agent + sub-agent orchestration

│   ├── tools.py               # MCP tool wrappers (Calendar, Notes, etc.)

│   └── workflows.py           # Multi-step workflow definitions
...
│   ├── models.py              # SQLAlchemy ORM models (plans, workouts, diet, appointments, notes)
...
│   └── main.py                # FastAPI routes (e.g., /wellness_plan)
...
├── .env                       # Environment variables (PROJECT_ID, DB creds, service account)

├── requirements.txt           # Python dependencies

├── Dockerfile                 # Container build for Cloud Run

├── run.sh                     # Helper script to run locally
